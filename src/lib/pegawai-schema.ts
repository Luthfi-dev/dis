...
export const pegawaiFormSchema = z.object({
  ...
  pegawai_tanggalLahir: z.union([z.string().datetime(), z.date()]).transform(val => new Date(val)),
  ...
  pegawai_terhitungMulaiTanggal: z.union([z.string().datetime(), z.date()]).transform(val => new Date(val)),
